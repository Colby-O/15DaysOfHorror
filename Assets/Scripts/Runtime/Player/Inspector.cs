...
namespace DOH
{
    ...
    public class Inspector : MonoBehaviour
    {
        ...
        {
            GameManager.GetMonoSystem<IInputMonoSystem>().InteractCallback.AddListener(EndInspect);
            GameManager.GetMonoSystem<IInputMonoSystem>().RCallback.AddListener(ToggleRead);
        }

        private void OnDisable()
        {
            GameManager.GetMonoSystem<IInputMonoSystem>().InteractCallback.RemoveListener(EndInspect);
            GameManager.GetMonoSystem<IInputMonoSystem>().RCallback.RemoveListener(ToggleRead);
        }

        private void Awake()
        {
            if (!_playerController) _playerController = GetComponent<Controller>();
